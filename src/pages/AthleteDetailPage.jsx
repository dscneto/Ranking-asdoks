...
export default function AthleteDetailPage() {
  ...
  if (!athlete) return (
    <EmptyState title="Atleta não encontrado" action={<Button onClick={() => navigate('/atletas')}>Voltar</Button>} />
  )
  ...
  return (
    <div>
      <button onClick={() => navigate('/atletas')} className="flex items-center gap-1.5 text-[#1B4FA8] font-semibold text-sm mb-5 hover:underline">
        <EvaIcon name="arrow-back-outline" size={16} fill="currentColor" /> Voltar para Atletas
      </button>
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap">
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap mt-1.5">
            ...
            {athlete.training_unit_label && <Chip variant="default">{athlete.training_unit_label}</Chip>}
          </div>
        </div>
      </div>
      ...
      <h3 className="text-[11px] font-bold uppercase tracking-widest text-[#A8AFBC] mb-3">Histórico em competições</h3>

      {!history?.results?.length ? (
        <EmptyState title="Nenhum resultado lançado" description="Os resultados aparecerão aqui após serem lançados." />
      ) : (
        <div className="bg-white border border-[#DDE1EA] rounded-xl shadow-sm overflow-hidden">
          ...
          {history.results.map((r, i, arr) => {
            ...
            return (
              <div key={r.id} className={`grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1.5fr_1fr_1fr_auto] gap-1.5 md:gap-3 px-4 py-3 text-sm ${i < arr.length - 1 ? 'border-b border-[#DDE1EA]' : ''}`}>
