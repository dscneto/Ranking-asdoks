...
import { rankingApi, coachesApi, trainingUnitsApi } from '../utils/api'
...
export default function RankingPage() {
  ...
  const [filters, setFilters] = useState({
    gender: '', ageCategoryId: '', modality: '', coachId: '', trainingUnitId: ''
  })
  ...
  // Carrega coaches e unidades para os filtros (só logado)
  useEffect(() => {
    if (!isAuth) return
    Promise.all([coachesApi.getAll(), trainingUnitsApi.getAll()])
      .then(([c, u]) => { setCoaches(c); setUnits(u) })
      .catch(() => { })
  }, [isAuth])
      ...
      if (navigator.onLine) {
        ...
        try {
          const [athletes, results, competitions, competitionTypes] = await Promise.all([
            getAll('athletes'), getAll('results'),
            getAll('competitions'), getAll('competitionTypes'),
          ])

          let filtered = athletes
          if (filters.gender) filtered = filtered.filter(a => a.gender === filters.gender)
          ...
          if (filters.trainingUnitId) {
            filtered = filtered.filter(a => a.training_unit_id === filters.trainingUnitId)
          }
          ...
          const ranked = filtered.map(athlete => {
            ...
            const competitionsCount = new Set(results.filter(r => r.athlete_id === athlete.id).map(r => r.competition_id)).size
            return { ...athlete, age, total_points: totalPoints, competitions_count: competitionsCount, ageCategory }
  ...
  const clearFilters = () => {
    setFilters({ gender: '', ageCategoryId: '', modality: '', coachId: '', trainingUnitId: '' })
    setSearch('')
  }
    ...
    <div>
      ...
      <div className="bg-white border border-[#DDE1EA] rounded-xl shadow-sm p-4 mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {/* Filtros exclusivos para logados */}
          {isAuth && (
            <>
              <select className={selectCls} value={filters.coachId} onChange={e => setFilters(f => ({ ...f, coachId: e.target.value }))}>
                <option value="">Todos os professores</option>
                {coaches.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>

              <select className={selectCls} value={filters.trainingUnitId} onChange={e => setFilters(f => ({ ...f, trainingUnitId: e.target.value }))}>
      ...
      {!loading && !error && filteredRanking.length === 0 && (
        <EmptyState
          title={search ? `Nenhum atleta encontrado para "${search}"` : 'Nenhum atleta encontrado'}
